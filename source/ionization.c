...
/*  This is a new development mode
  ...
  else if (mode == IONMODE_MATRIX_BOOST)
  {
    spectral_estimators (xplasma);
    update_old_plasma_variables (xplasma);
    ireturn = xshot (xplasma, mode);
 ...
 * 	by solving for tyhe temperature and abundances which match
 *      heating and cooling.
 *
...
int
xshot (PlasmaPtr xplasma, int mode)
{
  ...
  if (modes.zeus_connect == TRUE || modes.fixed_temp == TRUE)
  {
    ...
    xxxplasma = xplasma;
    xzero_emit (te_old);        //But we do still want to compute all heating and cooling rates
  }
  else                          //Find a new teperature where heating and cooling match
  {
    te_new = xcalc_te (xplasma, 0.7 * te_old, 1.3 * te_old);    //compute the new t_e - no limits on where it can go
    ...
    xzero_emit (xplasma->t_e);  //Get the heating and cooling rates correctly for the new temperature
  }
...
*/
  ...
  {                             /* Then modify to an on the spot approx */
    if (nebular_concentrations (xplasma, NEBULARMODE_MATRIX_SPECTRALMODEL))
    {
...
double
xzero_emit2 (double t, void *params)
{
  return (xzero_emit (t));
}
...
xzero_emit (double t)
{
  ...
  if (xxxplasma->t_r > 10.)
  {                             /* Then modify to an on the spot approx */
    if (nebular_concentrations (xxxplasma, NEBULARMODE_MATRIX_SPECTRALMODEL))
    {
      Error ("ionization_on_the_spot: nebular_concentrations failed to converge\n");
      Error ("ionization_on_the_spot: j %8.2e t_e %8.2e t_r %8.2e w %8.2e nphot %i\n", xxxplasma->j, xxxplasma->t_e, xxxplasma->t_r,
             xxxplasma->w, xxxplasma->ntot);
    }
    if (xxxplasma->ne < 0 || VERY_BIG < xxxplasma->ne)
    {
      Error ("ionization_on_the_spot: ne = %8.2e out of range\n", xxxplasma->ne);
    }
  }
...
xcalc_te (PlasmaPtr xplasma, double tmin, double tmax)
{
  ...
  if ((z1 * z2 < 0.0))
  {                             // Then the interval is bracketed
    xplasma->t_e = zero_find (xzero_emit2, tmin, tmax, 50., &ierr);
    if (ierr)
    {
      Error ("calc_te: zero_find failed to find a temperature\n");
    }
